...
/**
 *   Namespace for the Gameplay Tags used in the Pac Project.
 *   UE_DECLARE_GAMEPLAY_TAG_EXTERN for the .h
 *   UE_DEFINE_GAMEPLAY_TAG_COMMENT for the .cpp
 */
namespace PacGameplayTags
{
	// Gameplay Tags 
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_MovementStopped);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_JumpNotAllowed);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_Sprint);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_IsReloading);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_PreventFromShooting);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_PreventFromReloading);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_CoyoteTime);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Gameplay_InDeath);

	// Cheat Tags
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Cheat_GodMode);
	
	// Effects Tags
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Effect_Kamikaze);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Effect_DoubleJump);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Effect_DamageBoost);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Effect_DamageImmunity);
	UE_DECLARE_GAMEPLAY_TAG_EXTERN(TAG_Effect_Propulsion);
}
...
enum class EGameplayTagEventType : uint8
{
	...
	AnyCountChange UMETA(DisplayName = "Any Count Change")
};

/** 
 * Struct that tracks the number/count of tag applications within it. Explicitly tracks the tags added or removed,
 * while simultaneously tracking the count of parent tags as well. Events/d are fired whenever the tag counts
 * of any tag (explicit or parent) are modified.
 */

struct PAC_API FGameplayTagCountContainer
{
	...
	*/
	 ...
	 */
	FOnGameplayEffectTagCountChanged& RegisterGameplayTagEvent(const FGameplayTag& Tag, EGameplayTagEventType EventType = EGameplayTagEventType::NewOrRemoved);
